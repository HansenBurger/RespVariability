    ...
    if file_name == "zdt" or file_name == "zds":
        ...
        with open(file_loc, "rb") as fid:
            for i in wave_header_name:
                wave_header[i] = np.fromfile(
                    fid, wave_header_info[i][0], wave_header_info[i][1]
                )

            for i in range(32):
                for j in data_engine_name:
                    data_engine[j].append(
                        np.fromfile(fid, data_engine_info[j][0], data_engine_info[j][1])
                    )

        # new_list = []
        # for i in data_engine["wcLabel"]:
        #     new_list.append("".join([chr(item) for item in i]))
        # data_engine["wcLabel"] = new_list
        ...
        with open(file_loc, "rb") as fid:
            for i in range(int(file_size / 1024)):
                for j in vent_para_name:
                    vent_para[j].append(
                        np.fromfile(fid, vent_para_info[j][0], vent_para_info[j][1])
                    )
